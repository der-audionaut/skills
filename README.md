...
Three families live here: `news-*` produces researched briefings, `plan-*` works on feature plans across their entire lifecycle, `tool-*` takes over everyday development chores — so far the Git rebase and the feasibility check of a dependency update.
...
| `skills/plan-coding` | `/plan-coding <plan> <schritt>` | Implements exactly one step of the plan, including findings rounds, and commits it — not the plan file, never on the default branch, never with a push |
...
| `skills/tool-dep-update-check` | `/tool-dep-update-check <dependency> <sourceVersion> <targetVersion> [branch]` | Checks whether a dependency can be updated from one version to another: locates it in manifest and lock, measures the jump against the registry, reads changelog and release notes for the whole range, matches breaking changes against the code's actual usage, checks platform and dependency conflicts, runs a trial update in a throwaway worktree and reports a verdict with evidence — without changing the project |

All eight skills answer in German unless something says otherwise. `/plan-create` takes a different output language as its last optional argument (`/plan-create "invoice export as CSV" english` — the free-text description goes in quotes when a language follows), the news skills take the modifier `sprache <language>` in the request, and `/plan-review`, `/plan-lint` and `/plan-coding` take no language at all: they answer in the language the plan is written in. The two tool skills always report in German. What the language covers and what it leaves alone is described under [Conventions](#conventions).
...
The agent may pull the news skills on its own whenever a request matches their description. The plan and tool skills carry `disable-model-invocation: true` — they only run when you start them explicitly, because they touch files, or in the case of `plan-coding` and `tool-git-rebase` the Git history, that are not their own. `tool-dep-update-check` changes nothing that lasts, but it installs packages and runs the project's build and tests, which is work you should trigger knowingly.
...
`/plan-coding` implements one step and, once its findings loop has terminated green, commits it — only the files the step changed, never the plan file. The plan stays modified and unstaged in your working tree until you commit it yourself, so several steps can go into one plan commit. The message comes from the `write-commit-message` skill when one is available in the session, otherwise it follows the style of the project's recent commits; it carries no plan reference and no trailers, because the mapping from commit to step lives in the plan's `## Umsetzungs-Historie`, which names SHA and subject. No commit is made on the default branch or a detached HEAD, while a merge, rebase or cherry-pick is in progress, when the index already held staged changes, when a file the step touched was already modified before the run, or when verification stayed red — the step is still implemented, reported and written into the plan, with the reason in the report's `Commit:` line. Paths that appeared during the run but belong to nobody, such as build artifacts or files a formatter touched, are not staged; the report lists them under „Nicht zugeordnet" for you to decide. If the default branch cannot be detected, the skill writes the plan first and then asks you before committing. It never pushes, never creates a branch and never stashes; the way back is `git reset --soft HEAD~1`, and every report names it.
...
`tool-dep-update-check` answers the question a dependency-update PR would otherwise raise in review: can this dependency go from one version to another in this project, and what would it take? `/tool-dep-update-check lodash 4.17.20 4.17.21` checks on the branch you are on; a fourth argument names another branch, which the skill reads via `git show` and `git grep` without switching. It finds the dependency in manifest and lock — aborting on ambiguous names such as `guzzle`, and turning a transitive dependency into the question of which parents allow the target — asks the registry whether the target exists, when it was published, what runtime and peers it requires and who else in the project pins the package, then reads migration guide, changelog and release notes for the entire range, because the breaks of a 4.9 → 5.2 jump sit in 5.0.0, not in 5.2.0. Every break is matched against the code's actual usage, including imports, subpath imports, configuration, strings and framework wiring, and every "not affected" carries the grep pattern that proved it. The per-ecosystem commands for npm, Composer, Maven, Gradle, pip and friends, Cargo, Go, Bundler and NuGet live in `references/oekosysteme.md`; the hiding places of undocumented breaks — changed defaults, stricter validation, ESM-only builds, renamed properties, raised peer ranges, license changes — in `references/bruchstellen.md`.

Static analysis says what should break; the trial says what does. The skill creates a detached worktree in a temp directory, installs the baseline from the lock, dry-runs the resolution, pins the exact target, runs the project's build, type check and tests, compares red results against the baseline so pre-existing failures are not blamed on the update, and removes the worktree again. It never repairs the worktree to get green — every fix it would make there is a finding it would hide — and it never performs the update in your working tree. The report ends with a verdict (possible, possible with adaptations, blocked — the last always with the precondition that would unblock it), a confidence line that says whether the trial ran and whether the tests cover the affected surface, and the command you would run for the real update.
...
```bash
rm ~/.claude/skills/{news-nachrichtenlage,news-wirtschafts-briefing,plan-create,plan-review,plan-lint,plan-coding,tool-git-rebase,tool-dep-update-check}
```
...
- `argument-hint` / `arguments` — named arguments, mapped positionally and usable in the text as `$plan`, `$schritt`, `$basebranch`, `$branch`, `$targetVersion`; an argument that is not passed becomes the empty string, which is why trailing arguments can be optional. The argument string is split like a shell command line, so a value with spaces stays one argument only inside quotes; `$ARGUMENTS` always holds the complete string as typed
...
- **German by default.** The instructions are written in German, and so are report and output unless a skill is told otherwise: `/plan-create` by its last argument, the news skills by the modifier `sprache <language>`, and the three downstream plan skills by the plan itself, whose language they follow. The two tool skills carry a fixed directive „Deutsch" in their first line. Wherever the output can differ from German, the language directive is the first line of the skill and is checked once more at the end: a single line in the middle of a German instruction demonstrably is not enough, the model answers in German anyway.
...
- **Every iterative skill needs a termination criterion.** Without one, an agent invents findings in round three to look busy. Hence the same pattern everywhere: no new optional findings from round 2 on, rejected findings stay rejected, after three rounds without progress the human decides — and a rebase that cannot get one commit through in three attempts stays standing for the human instead of running on with invented resolutions, just as a trial update that cannot get through install and build in three attempts for environmental reasons is reported as unverified instead of being guessed.
- **A skill stops where the decision is the human's.** The rebase never pushes, the update check never performs the update, and `plan-coding` commits its own step locally but never pushes, never touches the default branch and never stages what it did not change itself. Each report ends with the command the human would run next and, where the skill changed something, with the way back.
