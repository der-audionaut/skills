...
The agent may pull the news skills on its own whenever a request matches their description. The plan and tool skills carry `disable-model-invocation: true` — they only run when you start them explicitly, because they touch files, or in the case of `tool-git-rebase` the Git history, that are not their own. `tool-dep-update-check` changes nothing that lasts, but it installs packages and runs the project's build and tests, which is work you should trigger knowingly.
...
- **A tool skill stops where the decision is the human's.** The rebase never pushes, the update check never performs the update. Each report ends with the command the human would run next and, where the skill changed something, with the way back.
